...
public class AVLSanitizer {
	...
	public static int calcSum(AVLTree.IAVLNode node) {
		if (node == null || !node.isRealNode())
			return 0;
		return calcSum(node.getLeft()) + calcSum(node.getRight())
				+ node.getKey();
	}
	...
	public static boolean sanitizeTree(AVLTree.IAVLNode root) {
		...
		if (calcHeight(root) != root.getHeight()) {
			System.out.format("Height should be %d but is %d%n",
					calcHeight(root), root.getHeight());
			return false;
		}

		if (calcSize(root) != root.getSize()) // Typo here! Used to say
												// getHeight, but should be
												// getSize
		{
		 ...
		 * System.out.format("Sum should be %d but is %d%n", calcSum(root),
		 * root.getSubtreeSum()); return false; }
		 */
		int bf = calcHeight(root.getLeft()) - calcHeight(root.getRight());
		if (Math.abs(bf) >= 2) {
			System.out.format("Balance factor should be out of range %d%n", bf,
					root.getHeight());
			return false;
		}
